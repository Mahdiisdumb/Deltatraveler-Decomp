...
public class PaulaMeleeStarBullet : BulletBase
{
	...
	private void Update()
	{
		if (!hard)
		{
			...
			base.transform.localPosition = basePos + Vector3.Lerp(Vector3.zero, new Vector3(Mathf.Cos((float)angle * (MathF.PI / 180f)), Mathf.Sin((float)angle * (MathF.PI / 180f))) * 2f, num2);
		...
		{
			frames++;
			float t = Mathf.Sin((float)frames * (180f / reachout) * (MathF.PI / 180f));
			float t2 = (float)frames / reachout;
			angle -= (spinReverse ? (-2) : 2);
			base.transform.localPosition = basePos + Vector3.Lerp(Vector3.zero, new Vector3(Mathf.Cos((float)angle * (MathF.PI / 180f)), Mathf.Sin((float)angle * (MathF.PI / 180f))) * 4f, t);
	...
	public void SetValues(int i, Vector3 basePos, bool spinReverse, bool hard = false)
	{
		...
		if ((bool)Util.FindObjectOfType<PaulaDeathAttack>())
		{
			baseDmg = 1;
		}
	}
