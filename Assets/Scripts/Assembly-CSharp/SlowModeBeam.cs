...
public class SlowModeBeam : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		else if (frames >= 40 && frames < 70)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				float y = Vector3.Distance(Util.FindObjectOfType<SOUL>().transform.position, beams[j].transform.position) * 48f;
				...
				beams[j].transform.up = Util.FindObjectOfType<SOUL>().transform.position - beams[j].transform.position;
			}
		...
		else if (frames >= 70)
		{
			for (int k = 0; k < 2; k++)
			{
				float y2 = Vector3.Distance(Util.FindObjectOfType<SOUL>().transform.position, beams[k].transform.position) * 48f;
				...
				beams[k].transform.up = Util.FindObjectOfType<SOUL>().transform.position - beams[k].transform.position;
			}
			soulBall.transform.localScale = Vector3.Lerp(new Vector3(3f + num, 3f + num), Vector3.zero, (float)(frames - 70) / 10f);
		}
		soulBall.position = Util.FindObjectOfType<SOUL>().transform.position;
		if (frames == 100)
		{
