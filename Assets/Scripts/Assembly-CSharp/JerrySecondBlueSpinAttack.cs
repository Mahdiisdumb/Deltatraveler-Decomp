	...
	protected override void Update()
	{
		...
		else
		{
			...
			if (frames <= 65)
			{
				...
				swordSpinning.eulerAngles = new Vector3(0f, 0f, angle);
				float num6 = Mathf.Lerp(Mathf.Abs(Mathf.Sin(angle * (MathF.PI / 180f))), 0f, (float)frames / 65f);
				swordSpinning.localScale = new Vector3(1f + num6 / 5f, 1f - num6 / 2f, 1f);
				swordSpinning.position = new Vector3(swordSpinning.position.x, Mathf.Lerp(swordSpinning.position.y, Util.FindObjectOfType<SOUL>().transform.position.y, t));
			}
			if (frames > 65)
			{
				if (frames == 66)
