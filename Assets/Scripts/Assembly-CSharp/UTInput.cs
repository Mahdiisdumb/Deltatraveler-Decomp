...
public class UTInput : MonoBehaviour
{
	...
	public static bool GetButtonUp(string button)
	{
		...
		if (!flag && Keyboard.current != null)
		{
			...
			foreach (Key key in altKeys)
			{
				if (key != Key.None && Keyboard.current[key].wasReleasedThisFrame)
				{
					flag = true;
				}
			}
	...
	public static bool GetButton(string button)
	{
		...
		if (!flag && Keyboard.current != null)
		{
			...
			foreach (Key key in altKeys)
			{
				if (key != Key.None && Keyboard.current[key].isPressed)
				{
					flag = true;
				}
			}
	...
	private static void LoadInputsFromConfig(Config c)
	{
		...
		{
			string value = c.GetString("Keyboard Controls", inputNames[i], defaultKeyNames[i]);
			try
			{
				keys[inputNames[i]] = (Key)Enum.Parse(typeof(Key), value);
			}
			catch
			{
	...
	{
		string text = c.GetString("Keyboard Controls", inputName, "null");
		text.Replace("Control", "Ctrl");
		text.Replace("Return", "Enter");
		text.Replace("Keypad", "Numpad");
		text.Replace("Alpha", "Digit");
		text.Replace("BackQuote", "Backquote");
		try
		{
			Enum.Parse(typeof(Key), text);
			return text;
		}
		catch
		{
	...
	{
		string text = c.GetString("Gamepad Controls", inputName, "null");
		...
		{
			Debug.Log("found a replacement for " + text);
			return dictionary[text];
		}
		try
		{
			Enum.Parse(typeof(GamepadButton), text);
			return text;
		}
		catch
		{
