...
public class Gringus : EnemyBase
{
	...
	protected override void Awake()
	{
		...
		atk = 4;
		def = 6;
		flavorTxt = new string[4] { "* Gringus doesn't seem to\n  know why it's here.", "* Gringus hops to and fro.", "* The battlefield is filled\n  with the smell of\n  mustard seed.", "* You are intimidated by\n  Gringus's raw strength.^30\n* Only kidding." };
		dyingTxt = new string[1] { "* Gringus is trying to\n  run away." };
		satisfyTxt = new string[1] { "* Gringus seems reluctant\n  to fight you." };
		...
		susieMiniACTs = new MiniACT[2]
		{
			...
			new MiniACT("Gringus", 2, "Fuck you!")
		};
		noelleMiniACTs = new MiniACT[2]
		{
			EnemyBase.NACTION_DEFAULT,
			new MiniACT("Gringus", 2, "", 20)
		};
	...
	public override string[] PerformAct(int i)
	{
		...
		if (GetActNames()[i] == "Compliment")
		{
			...
			return new string[1] { "* Gringus didn't understand\n  what you said,^10 but was\n  flattered anyway." };
		}
		if (GetActNames()[i] == "Threat")
		{
			...
			return new string[2] { "* Gringus didn't understand\n  what you said,^10 but was\n  scared anyway.", "su_wideeye`snd_txtsus`* Kris???" };
		}
	...
	{
		if (partyMember == 1 && i == 2)
		{
			AddActPoints(50);
			return new string[1] { "* Gringus test worked!" };
		}
		if (partyMember == 2 && i == 2)
		{
			AddActPoints(51);
			return new string[1] { "* Gringus test worked!\n  (but for Noelle!)" };
		}
