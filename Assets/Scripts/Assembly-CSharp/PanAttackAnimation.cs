...
public class PanAttackAnimation : PlayerAttackAnimation
{
	...
	protected override void Update()
	{
		if (isPlaying)
		{
			...
			else
			{
				...
				for (int i = 0; i < 8; i++)
				{
					base.transform.GetChild(i).transform.localPosition = Vector3.Lerp(Vector3.zero, new Vector3(Mathf.Cos((float)(45 * i) * (MathF.PI / 180f)), Mathf.Sin((float)(45 * i) * (MathF.PI / 180f))) * 2f, num2);
	...
	public override void AssignValues(EnemyBase enemy, int partyMember, float targetExcellence, int partySize, int slot)
	{
		...
		color = PartyMembers.GetMemberNeonColor(partyMember);
